...
# joineamos los dataframes en uno solo


def join_df(lista_regiones):
    """
    Lee una lista de archivos csv y los concatena en un solo CSV.

    :param lista_regiones: lista de archivos csv
    """

    df = pd.concat(map(pd.read_csv, lista_regiones))
    df.to_csv(rf"../Dataframe/indice_aperturas_{fecha}.csv", index=False)


if __name__ == "__main__":
    extract(ipc_aperturas)
    transform_df_regiones(regiones, df)
    join_df(lista_regiones)
    print("Archivo transformado")
    print("CSVs de regiones creados\n", *lista_regiones, sep="\n")
    print("CSV indice de aperturas creado\n", f"indice_aperturas_{fecha}.csv")
